                            ...
                            double occ_thresh, double free_thresh)
{
    ...
    //if (mode==TRINARY)
    //{
        for (GridMapIterator iterator(grid); !iterator.isPastEnd(); ++iterator) {
            float value = (grid.at(layer, *iterator) - dataMin) / (dataMax - dataMin);
            if (value < free_thresh)
                value = dataMin;
            else if (value > occ_thresh)
                value = dataMax;
            else 
                value = nanf("");
        ...
        // Use Eigen operations to convert data between 0 and 1
        // Then set all cells between free_thresh & occ_thresh to NaN 
    //}
    */
